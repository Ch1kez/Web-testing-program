...
import requests
import socket
from srFTA import get_data_from_bd


fta_bd = r'C:\Users\123\PycharmProjects\api_on_postgre\NEW_Py_FTA\BD\ftacommander.db'
#fta_bd = r'd:\Ivanov\selenium\BD\ftacommander.db'
...
def get_par_test(t_array, trgp):
	...
	int_ip = sel_sql("SELECT {1}adr FROM Target where tname = '{0}'".format(retv['tsts'], 'd' if retv['dbg'] else 'm') )[0][0]
...
def save_data(file_name, file_data):
	with open(file_name, 'w', encoding="utf-8") as out_file:
		out_file.write(json.dumps(file_data, ensure_ascii=False, indent=2))
...
def getUserCred(credVal, userN, target):
	sql = "select {0} retv from tuser where target = '{2}' and uorg = '{1}';".format('ulogin' if credVal == 'login' else 'upass', userN, target)
	usrrec = sel_sql(sql)
	return usrrec[0][0]
...
def prep_send_keys(inp_txt, tst_info):
	...
	if retV.startswith('@@@'):
		...
		if cmd[1] == "now":  # --- Формируем дату
			...
			retV = getUserCred(cmd[2], cmd[3], tst_info['tsts'])
		elif cmd[1] == "ИнфЭОТТС":  # +++
			retV = cmd[2]
		elif cmd[1] == "ЭПТС":  # +++
			retV = inp_txt
			...
			numQ = "1"
			if not cmd[1] == "КодПодтверждения":
				numQ = cmd[1].substring(len(cmd[1]) - 1)
				raise Exception("Не проверено")
			if cmd[3] == '':
				retV = get_data_from_bd(tst_info['var_arr'][cmd[2]], numQ, tst_info['tsts'])["rcode"]
			elif cmd[3] == 'залога':
				retV = get_data_from_bd(tst_info['var_arr'][cmd[2]], numQ, tst_info['tsts'])["zcode"]
			elif cmd[3] == 'доступа':
				retV = get_data_from_bd(tst_info['var_arr'][cmd[2]], numQ, tst_info['tsts'])["dcode"]
			# raise Exception("Не реализовано")
...
def create_pts_web(tst_inf, pts_mask):
	qwrMask = pts_mask.split("@@@")
	ip = "10.17.15.45" if tst_inf['dbg'] else "192.168.135.215"
	trg = "243" if str(tst_inf['tsts']).endswith('243') else "240"
